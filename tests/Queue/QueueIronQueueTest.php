...
class QueueIronQueueTest extends PHPUnit_Framework_TestCase {
	...
	public function testDelayedPushProperlyPushesJobOntoIronWithTimestamp()
	{
		...
		$queue = $this->getMock('Illuminate\Queue\IronQueue', ['getTime'], [$iron = m::mock('IronMQ'), m::mock('Illuminate\Http\Request'), 'default', true]);
		$crypt = m::mock('Illuminate\Contracts\Encryption\Encrypter');
		$queue->setEncrypter($crypt);
	...
	{
		$queue = $this->getMock('Illuminate\Queue\IronQueue', ['createPushedIronJob'], [$iron = m::mock('IronMQ'), $request = m::mock('Illuminate\Http\Request'), 'default', true]);
		$crypt = m::mock('Illuminate\Contracts\Encryption\Encrypter');
		$queue->setEncrypter($crypt);
